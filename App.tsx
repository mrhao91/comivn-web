...
import React, { useEffect, useState, createContext, useContext, useCallback, Suspense } from 'react';
...
const ComicDetail = React.lazy(() => import('./pages/ComicDetail'));
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const loadTheme = async () => {
        ...
        root.style.setProperty('--color-primary', theme.primaryColor || '#b45309');
        ...
        root.style.setProperty('--footer-text', theme.footerText || '#cbd5e1');
        ...
        if (theme.fontFamily) {
            ...
            if (['Roboto', 'Open Sans', 'Patrick Hand', 'Playfair Display', 'Merriweather', 'Comfortaa'].includes(fontName)) {
                ...
                if (fontName === 'Playfair Display' || fontName === 'Merriweather') fontStack = `'${fontName}', serif`;
                if (fontName === 'Patrick Hand') fontStack = `'${fontName}', cursive`;
  ...
  return (
    <LeechProvider>
        <BrowserRouter>
            <LeechProgressWidget />
            <Suspense fallback={<div className="min-h-screen flex items-center justify-center bg-darker text-primary">Đang tải trang...</div>}>
                <Routes>
